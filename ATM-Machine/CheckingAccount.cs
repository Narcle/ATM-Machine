        ...
        public decimal Fee
        {
            ...
            set
            {
                if (value > 0)
                    ...
                    throw new Exception("Fee cannont be less than zero");
            }
        }
        // Class CheckingAccount should redefine methods Credit and Debit so that they subtract the fee from the account balance
        // whenever either transaction is performed successfully. CheckingAccount’s versions of these methods should invoke the
        // base-class Account version to perform the updates to an account balance. CheckingAccount’s Debit method should charge
        // a fee only if money is actually withdrawn (i.e., the debit amount does not exceed the account balance).
